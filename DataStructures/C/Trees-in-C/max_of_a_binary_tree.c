...
/*
 * Find max of tree using recursion
 * Auxillary Space = O(height of tree)
 */

int findMax(binary_tree_t* root)
{
    if(root == NULL)
    {
        return INT_MIN;
...
/* 
 * Find max using Level-order Traversal of the Binary Tree 
 * Auxillary space -> O(width of tree)  
 */

int iterative_FindMax(binary_tree_t* root, queue_t* Q)
{
    qpush(Q, root);
...
int main(void)
{
    ...
    {
        /* Display the tree */ 
        printf("\r\nMax of tree using both algorithms: ");
        printf("%d ", iterative_FindMax(tree, &Q));
