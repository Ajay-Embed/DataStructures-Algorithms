...
/** Recursive implementation to print nodes at distance k in a tree **/ 

void print_nodes_at_k(binary_tree_t* root, int k)
{
    if(root == NULL)
    {
        return;
...
int main(void)
{
    ...
    else
    {
        ...
        printf("\r\nNodes at K distance are= ");
        print_nodes_at_k(tree, k);
    }
