...
/** (This is why C++'s STL is helpful in such problems) **/
...
/* Main solution
 * This function prints the level order traversal of the binary tree 
 * with each level beginning from a new line, uses NULL as a delimiter.
 */
...
void display_levelorder(binary_tree_t* root, queue_t* Q)
{
    ...
    while(Qsize(Q) > 1)
    {
        ...
        /* If we see a delimiter print new line */
        if(node == NULL)
        {
            qpush(Q, NULL);
            printf("\r\n");
...
void print_left(binary_tree_t* node, int level)
{
    if(node == NULL) 
        return; 
    ...
    if(node->left)
    {
        print_left(node->left, level+1);
    }
    
    if(node->right)
    {
        print_left(node->right, level+1);
    }           
}
 ...
 * Space Complexity = O(n) [Technically theta(w)]
 */
queue_t Q;
...
int main(void)
{
    ...
    else
    {
        ...
        print_left(tree, 1); //display_levelorder(tree, &Q);
    }
